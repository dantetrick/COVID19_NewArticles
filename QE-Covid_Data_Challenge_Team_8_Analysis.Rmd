...
Creates a data frame and retains columns identified by ColumnsToKeep listed above. 
NOTE: Takes about 30 minutes and is ~10GB 

```{r, warning=F}
dt_datastreams <- lapply(ls_data, function(x){ data.frame(stream_in(x))[,ColumnsToKeep]})
```
...
#### Write out data
```{r, warning=F, message=F}
fwrite(dt_datastreams, "E:/COVID19/ENGLISH NEWS ARTICLES THAT MENTION CORONA VIRUS.csv")

```

#### Filter for foxnews.com and cnn.com
```{r}

dt_samp <- dt_datastreams[thread.site =="foxnews.com"
                                 | thread.site =="cnn.com",]

fwrite(dt_samp, "E:/COVID19/ENGLISH NEWS ARTICLES THAT MENTION CORONA VIRUS FOX AND CNN.csv")
```
